...
while(WeightCount < inventory_size-1) {
	InvWeightAdder[0] += ds_grid_get(inventory,WeightCount,1)*ds_grid_get(inventory,WeightCount,2)
	InvWeightAdder[1] += ds_grid_get(grd_inv_armor,WeightCount,1)
	InvWeightAdder[2] += ds_grid_get(grd_inv_ammo,WeightCount,1)*ds_grid_get(grd_inv_ammo,WeightCount,2)
	InvWeightAdder[3] += ds_grid_get(grd_inv_aid,WeightCount,1)*ds_grid_get(grd_inv_aid,WeightCount,2)
	WeightCount += 1
}
...
if(WeightCount = inventory_size-1) {
	InvWeight = InvWeightAdder[0]+(InvWeightAdder[1]-Head-Torso-ArmL-ArmR-LegL-LegR)+InvWeightAdder[2]+InvWeightAdder[3]  
